...
echo_latest_version() {
  if [ "${EDGE-}" ]; then
    ...
    version="$(curl -fsSLI -o /dev/null -w "%{url_effective}" https://github.rc1844.workers.dev/https://github.com/coder/code-server/releases/latest)"
  fi
  version="${version#https://github.rc1844.workers.dev/https://github.com/coder/code-server/releases/tag/}"
  version="${version#v}"
  echo "$version"
}
